...
- **Phase 0 (현재):** 골격 — 정규형·어댑터 계약·스텁·Signal Gate 1차 라우팅.
- **Phase 1:** 어댑터 실동작(spawn/parse) + Retry Lane 실동작 + Quarantine 영속화.
- **Phase 2:** AI Triage(분류·클러스터링·confidence·Human Queue).
- **Phase 3:** Governance Console(정책·승인·audit·대시보드).
- **Phase 4:** Remediation(test_only 먼저 → app_source).
- **Phase 5:** Authoring(AI TC 생성).

> ③ 자동 수정을 의도적으로 맨 뒤에 둔다. 분류·격리·거버넌스로 신뢰를 쌓기 전에 자동 수정을 켜면 노이즈를 코드로 증폭시킨다.
